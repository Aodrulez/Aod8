...
void run(unsigned int ip_start,unsigned int ip_stop)
{
IP=ip_start;

	while(IP>=0 && OPCODE!=0x3c && IP<ip_stop)
	{
	OPCODE=ROM[IP];
...
void disasm(unsigned int ip_start,unsigned int ip_stop)
{
int IP=ip_start;
int SP,A,B,FLAG;
	while(IP>=0 && OPCODE!=0x3c && IP<ip_stop)
